...
## Product screenshots
- Real product screenshots live in `src/assets/screenshots/*.{png,webp}` (synthetic demo data), resolved by name via **`lib/screenshots.ts`** (`screenshotByName`) — the **one** glob, shared by `Screenshot.astro` and `ProductTour.astro`. Covers are PNG; gallery-only shots are pre-resized **WebP** (source PNGs live in `~/Downloads/lucoze-images`; convert with `sharp`, `.resize({width:1600}).webp({quality:80})`, ~34-83KB). Render via **`Screenshot.astro`** (`name` = filename w/o ext) — `astro:assets` `<Image>` for auto WebP/AVIF + responsive srcset + no-CLS + lazy. Never put product images in `public/` or use raw `<img>`.
- **Homepage product tour = `ProductTour.astro`** (`data/product-tour.ts` = 6 flows × shots). Clickable flow tiles open one native `<dialog>` lightbox (prev/next, arrow keys, swipe, counter, Esc + backdrop close). Gallery images are `loading="lazy"` inside the hidden dialog → they cost nothing until a flow is opened. `.tour-grid` (3-up responsive) lives once in `sections.css`. Add/reorder flows by editing `TOUR_FLOWS`.
