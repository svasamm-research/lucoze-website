...
- **Per-flow galleries = `ProductGalleries.astro`** (reusable; takes a `flows: string[]` prop). Data in **`data/flow-galleries.ts`** (`GALLERIES`, keyed by flow id, 3 shots each). Renders clickable cover tiles + one native `<dialog>` lightbox (prev/next, arrow keys, swipe, counter, Esc + **click-outside-image** close; images `loading="lazy"` in the hidden dialog). **Placement**: galleries live *in context* — feature pages via the `FEATURE_GALLERY` slug→id map in `features/[slug].astro` (appointments→front-office, patient-records→clinical, lab, pharmacy, billing); IPD/OT/emergency/insurance on `solutions/hospitals.astro`. The **homepage stays light** — hero + specialty switcher + a `.platform-explore` link-out (no galleries). Use `ProductGalleries` **once per page** (single `#galleryLightbox`).
