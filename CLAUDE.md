...
## Build / test / verify
- `npm run build` (astro) · `npm test` (jest) · **`npm run test:e2e`** (Playwright smokes vs the built site — ROI island, mobile overflow, lead form, clean-blog-`<p>`, routes, hero/fonts, tour lightbox) · **`npm run test:infra`** (nginx redirects + headers, needs Docker) · `npm run dev` (localhost:4321).
