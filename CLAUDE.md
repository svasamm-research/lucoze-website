...
- **Per-flow galleries = `ProductGalleries.astro`** (reusable; `flows: string[]` prop + `tiles?: boolean`). Data in **`data/flow-galleries.ts`** (`GALLERIES`, keyed by flow id, 3 shots each). Renders clickable cover tiles (browser-chrome frame + "N screens" badge) + one native `<dialog>` lightbox (prev/next, arrow keys, swipe, counter, Esc + **click-outside-image** close; images `loading="lazy"` in the hidden dialog). Use **once per page** (single `#galleryLightbox`). **Placement — galleries live in context, never crowd the homepage:**
  - **Feature pages** (`features/[slug].astro`): the `FEATURE_GALLERY` slug→id map (appointments→front-office, patient-records→clinical, lab, pharmacy, billing) feeds the gallery into the **SubHero `visual` slot** (hero right side), not a separate section.
