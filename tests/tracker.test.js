...
describe("LucozeTracker", () => {
	...
	test("fires engaged event after 60 seconds", () => {
		...
		const allEvents = fetchCalls.flatMap((c) => {
			const body = c[1].body;
			// body is FormData — extract events field
			const eventsStr = body instanceof FormData ? body.get("events") : JSON.parse(body).events;
